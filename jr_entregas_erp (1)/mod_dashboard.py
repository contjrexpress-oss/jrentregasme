...
# Cores corporativas
COR_AZUL = "#0B132B"
COR_AZUL_CLARO = "#1C2541"
COR_LARANJA = "#F29F05"
COR_LARANJA_ESCURO = "#D98E04"
COR_VERDE = "#38A169"
COR_VERMELHO = "#E53E3E"
COR_AZUL_MEDIO = "#3182CE"
COR_CINZA = "#718096"

CORES_REGIOES = [COR_LARANJA, COR_AZUL, COR_AZUL_CLARO, COR_VERDE, COR_AZUL_MEDIO]
CORES_CUSTOS = [COR_LARANJA, COR_AZUL_MEDIO, COR_VERDE, COR_VERMELHO, "#805AD5", "#DD6B20", "#319795"]
...
def _render_metricas(dados):
    ...
    with col6:
        st.markdown(metric_card(
            ...
            f"R$ {faturamento_mes:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."),
            "metric-green"
        ), unsafe_allow_html=True)
    with col7:
        st.markdown(metric_card(
            f"Custos ({dados['mes_label'][:3]})",
            f"R$ {custos_total_mes:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."),
            "metric-red"
        ), unsafe_allow_html=True)
    with col8:
        ...
        st.markdown(metric_card(
            ...
            f"R$ {lucro_mes:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."),
            lucro_class
        ), unsafe_allow_html=True)
...
def _render_alertas_contas():
    ...
    <div style='display: flex; gap: 20px; flex-wrap: wrap;'>
        <span style='color: {COR_VERMELHO}; font-weight: 600;'>A Pagar: R$ {total_pagar:,.2f}</span>
        <span style='color: {COR_VERDE}; font-weight: 600;'>A Receber: R$ {total_receber:,.2f}</span>
    </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    for conta in contas[:10]:
        ...
        padding: 10px 16px; margin: 5px 0; display: flex; align-items: center; justify-content: space-between;'>
            ...
            <div style='text-align: right; display: flex; align-items: center; gap: 10px;'>
                <span style='color: {COR_AZUL}; font-weight: bold;'>R$ {conta['valor']:,.2f}</span>
