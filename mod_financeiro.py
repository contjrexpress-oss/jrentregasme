...
def _render_autocomplete_cliente(label="Cliente", key_suffix="", placeholder_nenhum="(Nenhum)", incluir_bairro=False):
    ...
    Args:
        label: Label do campo de busca.
        key_suffix: Sufixo para keys únicos (ex: "faturamento", "custo").
        placeholder_nenhum: Texto do placeholder quando nenhum selecionado.
        incluir_bairro: Se True, inclui bairro no label do selectbox.
    
    Returns:
        Tuple[Optional[int], str]: (cliente_id, cliente_nome)
    """
    # Campo de busca fora do form para permitir filtragem dinâmica
    busca = st.text_input(
        f"🔍 Buscar {label} (nome ou CPF/CNPJ)",
        key=f"busca_cliente_{key_suffix}",
        placeholder="Digite para filtrar clientes cadastrados...",
        help="Digite pelo menos 2 caracteres para buscar. Filtra por nome ou CPF/CNPJ."
    )
    ...
    # Extrair cliente_id e nome
    if selecionado in mapa:
        cli = mapa[selecionado]
        return cli['id'], cli['nome']
    
    return None, ""


def _render_metricas_resumo(metricas):
    """Renderiza cards de métricas em colunas.
    metricas: lista de (titulo, valor_formatado, css_class)
    """
    cols = st.columns(len(metricas))
    for col, (titulo, valor, css_class) in zip(cols, metricas):
        with col:
            st.markdown(metric_card(titulo, valor, css_class), unsafe_allow_html=True)


def _validar_lancamento(descricao, valor, tipo_label="lançamento"):
    """Valida campos obrigatórios de descrição e valor.
    Retorna True se válido, False se inválido (exibe erro via st.error).
    """
    if not descricao:
        st.error(f"❌ Informe a descrição do {tipo_label}.")
        return False
    if valor <= 0:
...
def _render_seletor_item(items, tipo_label, key_prefix):
    """Selectbox genérico para escolher item por ID-Descrição-Valor.
    Retorna (sel_id, item_selecionado).
    """
    sel_id = st.selectbox(
        f"Selecionar {tipo_label}",
        options=[i['id'] for i in items],
        format_func=lambda x: (
            f"ID {x} - "
            f"{next((i['descricao'] for i in items if i['id'] == x), '')} - "
            f"{formatar_moeda_br(next((i['valor'] for i in items if i['id'] == x), 0))}"
        ),
        key=f"sel_{key_prefix}_editar"
    )
    return sel_id, next((i for i in items if i['id'] == sel_id), None)


def _render_botao_excluir(item_id, delete_fn, label, key):
    """Botão de exclusão com verificação de permissão (pode_excluir)."""
    if pode_excluir():
        if st.button(f"🗑️ Excluir {label}", key=key, type="secondary"):
            delete_fn(item_id)
            st.success(f"✅ {label} excluído!")
            st.rerun()


def _render_tabela_dados(df, colunas, rename_map, column_config=None):
    """Exibe tabela formatada com colunas selecionadas e renomeadas.
    Retorna o DataFrame de exibição.
    """
    cols_available = [c for c in colunas if c in df.columns]
    ...
    st.dataframe(df_display, use_container_width=True, hide_index=True, column_config=column_config or {})
    return df_display


# Helper legado mantido para compatibilidade (usado em _render_faturamento)
...
def _render_faturamento():
    ...
    with st.form("form_novo_faturamento"):
        ...
        if submit_fat:
            if not _validar_lancamento(fat_desc, fat_valor, "faturamento"):
                ...
                msg = f"✅ Faturamento adicionado! Valor: {formatar_moeda_br(fat_valor)}"
                if custos_assoc:
                    msg += f" | Custos: {formatar_moeda_br(total_custos)} | Lucro: {formatar_moeda_br(lucro_previsto)}"
                st.success(msg)
                st.rerun()
    ...
    _render_metricas_resumo([
        ("Total Faturamento", formatar_moeda_br(total_fat), "metric-green"),
        ("Total de Registros", len(df), "metric-blue"),
        ("Média por Nota", formatar_moeda_br(media), "metric-orange"),
        ("Lucro Líquido", formatar_moeda_br(lucro_total), lucro_class),
    ])
    ...
    if dados_lucro:
        ...
        if 'cliente_nome' in df_lucro.columns:
            df_lucro['cliente'] = df_lucro.apply(
                lambda r: r.get('cliente_nome') or r.get('cliente', ''), axis=1
            )

        _render_tabela_dados(df_lucro, cols_show, rename_map, column_config={
    ...
    with col_action[0]:
        fat_sel_id, fat_sel = _render_seletor_item(faturamento, "faturamento", "fat")
    with col_action[1]:
        if fat_sel:
    ...
    if fat_sel:
        ...
        _render_botao_excluir(fat_sel_id, deletar_faturamento, "Faturamento Selecionado", "btn_del_fat")
...
def _render_custos():
    ...
    for f in faturamentos:
        label = f"ID {f['id']} - {f.get('descricao', '')} - {formatar_moeda_br(f.get('valor', 0))} ({f.get('data', '')})"
        fat_opcoes.append(label)
        fat_map[label] = f['id']

    st.markdown("##### ➕ Novo Lançamento de Custo")

    # Autocomplete de cliente (fora do form para busca dinâmica)
    st.markdown("###### 👤 Selecionar Cliente (opcional)")
    ...
    with st.form("form_novo_custo"):
        ...
        if submit_custo:
            if not _validar_lancamento(desc_custo, valor_custo, "custo"):
                pass  # Erros já exibidos pela função
            else:
                ...
                # Determinar cliente_id (do autocomplete fora do form)
                cliente_id_custo = custo_cliente_id
                ...
                msg = f"✅ Custo adicionado: {formatar_moeda_br(valor_custo)}"
                if cliente_id_custo:
                    msg += f" — Cliente: {custo_cliente_nome}"
    ...
    # Métricas unificadas
    _render_metricas_resumo([
        ("Total de Custos", formatar_moeda_br(total_custos), "metric-red"),
        ("Total de Registros", len(df), "metric-blue"),
        ("Média por Custo", formatar_moeda_br(media_c), "metric-orange"),
    ])
    ...
    if custo_sel:
        ...
        _render_botao_excluir(custo_sel_id, deletar_custo, "Custo Selecionado", "btn_del_custo")
...
def _render_contas():
    ...
    with st.form("form_nova_conta"):
        ...
        if st.form_submit_button("➕ Registrar Conta", type="primary", use_container_width=True):
            if not _validar_lancamento(conta_desc, conta_valor, "conta"):
                pass
            else:
    ...
    # Métricas unificadas
    _render_metricas_resumo([
        ("Total a Pagar", formatar_moeda_br(total_pagar), "metric-red"),
        ("Total a Receber", formatar_moeda_br(total_receber), "metric-green"),
        ("Pendentes", str(pendentes), "metric-orange"),
        ("Atrasadas", str(atrasadas), "metric-red"),
    ])
    ...
    df_display['Valor (R$)'] = df_display['Valor (R$)'].apply(lambda x: formatar_moeda_br(x))
    ...
    st.markdown(f"**📊 Total exibido: {formatar_moeda_br(total_val)}** ({len(df_contas)} registros)")
        ...
        for conta in contas_pendentes_list:
            ...
            <div style="border-left: 4px solid {cor}; padding: 8px 14px; margin: 6px 0; 
                ...
                <span style="font-size: 0.9em;">
                    ...
                    <strong style="color: {cor};">{formatar_moeda_br(conta['valor'])}</strong> — 
    ...
    if contas_finalizadas:
        ...
        for conta in contas_finalizadas:
            ...
            tipo_icon = "💸" if conta['tipo'] == 'pagar' else "💰"
            with st.expander(f"{icon} {tipo_icon} {conta['descricao']} — {formatar_moeda_br(conta['valor'])} — {conta['status'].upper()}"):
                col1, col2 = st.columns(2)
                with col1:
                    st.markdown(f"**Tipo:** {'A Pagar' if conta['tipo'] == 'pagar' else 'A Receber'}")
                    st.markdown(f"**Valor:** {formatar_moeda_br(conta['valor'])}")
                with col2:
                    pag_str = ""
...
def _render_relatorios():
    ...
    _render_metricas_resumo([
        ("Receita Total", formatar_moeda_br(total_receita), "metric-green"),
        ("Custos Totais", formatar_moeda_br(total_custo), "metric-red"),
        ("Lucro Líquido", formatar_moeda_br(lucro), lucro_class),
        ("Margem de Lucro", f"{margem:.1f}%", "metric-blue"),
    ])
    ...
    if todas_pendentes:
        ...
        _render_metricas_resumo([
            ("A Pagar (pendente)", formatar_moeda_br(pagar_pend), "metric-red"),
            ("A Receber (pendente)", formatar_moeda_br(receber_pend), "metric-green"),
            ("Saldo Previsto", formatar_moeda_br(saldo), saldo_class),
        ])
    ...
    if not df_cus.empty and 'categoria' in df_cus.columns:
        ...
        with st.expander(f"📋 Lançamentos — {cat_filtro} ({len(df_drill)} registros, {formatar_moeda_br(df_drill['valor'].sum())})", expanded=False):
            ...
            df_drill_show = _render_tabela_dados(df_drill, drill_cols, rename)
            st.markdown(f"**Total: {formatar_moeda_br(df_drill['valor'].sum())}**")
    else:
    ...
    if not fat_mensal.empty or not cus_mensal.empty:
        ...
        with st.expander("📋 Tabela Resumo Mensal"):
            ...
            for col in ['Faturamento', 'Custos', 'Lucro']:
                df_mensal_show[col] = df_mensal_show[col].apply(lambda x: formatar_moeda_br(x))
    ...
    with col_g2:
        ...
        if not df_fat.empty and 'veiculo' in df_fat.columns:
            ...
            fat_veic_show['Valor'] = fat_veic_show['Valor'].apply(lambda x: formatar_moeda_br(x))
    ...
    if not df_fat.empty and 'cliente' in df_fat.columns:
        ...
        if not clientes_com_dados.empty:
            ...
            with st.expander(f"📋 Tabela completa — {len(fat_cliente)} clientes"):
                ...
                fat_cliente_d['Valor Total'] = fat_cliente_d['Valor Total'].apply(lambda x: formatar_moeda_br(x))
                st.dataframe(fat_cliente_d, use_container_width=True, hide_index=True)
                st.markdown(f"**Total: {formatar_moeda_br(clientes_com_dados['valor'].sum())}**")
    ...
    if not df_fat.empty and 'cliente' in df_fat.columns:
        ...
        if not clientes_fat_data.empty:
            ...
            with st.expander(f"📋 Tabela Lucro por Cliente — {len(lucro_cli)} clientes"):
                ...
                for col in ['Faturamento', 'Custos', 'Lucro']:
                    lucro_cli_show[col] = lucro_cli_show[col].apply(lambda x: formatar_moeda_br(x))
    ...
    df_mensal_pdf = df_mensal.copy() if df_mensal is not None and not df_mensal.empty else None
    ...
    nome_rel = f"relatorio_financeiro_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf"
    st.download_button(
