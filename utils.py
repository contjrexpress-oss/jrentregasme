...
def formatar_moeda_br(valor: float, prefixo: str = "R$ ") -> str:
    """Formata valor numérico para moeda brasileira.
    
    Args:
        valor: Valor numérico a ser formatado.
        prefixo: Prefixo da moeda (padrão: "R$ ").
    ...
    Examples:
        ...
        >>> formatar_moeda_br(0)
        'R$ 0,00'
        >>> formatar_moeda_br(-500.5)
        'R$ -500,50'
        >>> formatar_moeda_br(1234.56, prefixo="")
        '1.234,56'
    """
    try:
        valor = float(valor)
    ...
    # Usa formato americano e depois converte para brasileiro
    formatted = f"{valor:,.2f}"
    # Troca . por @ temporariamente, depois , por . e @ por ,
    formatted = formatted.replace(",", "@").replace(".", ",").replace("@", ".")
    return f"{prefixo}{formatted}"


def formatar_moeda_br_int(valor: float, prefixo: str = "R$ ") -> str:
    """Formata valor numérico para moeda brasileira sem casas decimais.
...
def validar_data(data_str):
    ...
    try:
        ...
        if dt.year < ANO_MINIMO:
            return False, f"Data '{data_str}' é anterior a {ANO_MINIMO}"
        return True, "OK"
    except ValueError:
...
def validar_cpf(cpf_str):
    """Valida CPF (11 dígitos). Retorna (bool, mensagem)."""
    if not cpf_str:
        return False, "CPF não informado"
    cpf = re.sub(r'\D', '', str(cpf_str))
    if len(cpf) != 11:
        return False, f"CPF deve ter 11 dígitos (encontrou {len(cpf)})"
...
def validar_cnpj(cnpj_str):
    """Valida CNPJ (14 dígitos). Retorna (bool, mensagem)."""
    if not cnpj_str:
        return False, "CNPJ não informado"
    cnpj = re.sub(r'\D', '', str(cnpj_str))
    if len(cnpj) != 14:
        return False, f"CNPJ deve ter 14 dígitos (encontrou {len(cnpj)})"
    # Verifica se todos os dígitos são iguais
    if cnpj == cnpj[0] * 14:
...
def validar_cpf_cnpj(documento):
    """Valida CPF ou CNPJ automaticamente baseado no tamanho. Retorna (bool, mensagem, tipo)."""
    if not documento:
        return True, "Documento não informado (opcional)", None
    doc_limpo = re.sub(r'\D', '', str(documento))
    if not doc_limpo:
        return True, "Documento não informado (opcional)", None
    if len(doc_limpo) <= 11:
        ok, msg = validar_cpf(doc_limpo)
        return ok, msg, "CPF"
    else:
        ok, msg = validar_cnpj(doc_limpo)
        return ok, msg, "CNPJ"
...
def validar_email(email: str) -> Tuple[bool, str]:
    """Valida formato de e-mail. Retorna (bool, mensagem)."""
    if not email or not str(email).strip():
        return True, "E-mail não informado (opcional)"
...
def validar_cliente_importacao(row: Dict[str, Any]) -> Tuple[bool, List[str]]:
    """Valida dados de um cliente para importação em lote.
    ...
    if documento:
        ok, msg, tipo = validar_cpf_cnpj(documento)
        if not ok:
            erros.append(msg)
    ...
    if telefone:
        tel_digitos = re.sub(r'\D', '', telefone)
        if tel_digitos and len(tel_digitos) < 10:
            erros.append(f"Telefone inválido: '{telefone}' (mínimo 10 dígitos)")
    
    return (len(erros) == 0, erros)


def extrair_clientes_csv(arquivo) -> Tuple[List[Dict[str, Any]], str]:
    """Extrai dados de clientes de arquivo CSV.
    
    Returns:
        Tuple (lista_de_dicts, mensagem_erro_ou_vazio)
    """
    import pandas as pd
    try:
        ...
        for enc in ['utf-8', 'latin-1', 'cp1252']:
            try:
                arquivo.seek(0)
                df = pd.read_csv(arquivo, encoding=enc, dtype=str)
                break
            except (UnicodeDecodeError, Exception):
                continue
        else:
            return [], "Não foi possível ler o arquivo CSV (encoding não suportado)"
        
        return _normalizar_dataframe_clientes(df), ""
    except Exception as e:
        return [], f"Erro ao ler CSV: {str(e)}"


def extrair_clientes_xlsx(arquivo) -> Tuple[List[Dict[str, Any]], str]:
    """Extrai dados de clientes de arquivo Excel (XLSX/XLS).
...
def extrair_clientes_pdf(arquivo) -> Tuple[List[Dict[str, Any]], str]:
    """Extrai dados de clientes de arquivo PDF (tabelas).
                            ...
                            for row in table[1:]:
                                if row:
                                    ...
                                    for i, cell in enumerate(row):
                                        if i < len(header):
                                            row_dict[header[i]] = str(cell).strip() if cell else ''
...
def extrair_clientes_imagem(arquivo) -> Tuple[List[Dict[str, Any]], str]:
    """Extrai dados de clientes de imagem usando OCR (pytesseract).
    ...
    try:
        ...
        # Tentar extrair como tabela usando pytesseract com TSV output
        tsv_text = pytesseract.image_to_data(img, lang='por', output_type=pytesseract.Output.DATAFRAME)
        
        # Agrupar texto por blocos/linhas
        text = pytesseract.image_to_string(img, lang='por')
        
        if not text or not text.strip():
            return [], "Nenhum texto encontrado na imagem (OCR)"
        
        # Tentar interpretar como tabela
        lines = [l.strip() for l in text.split('\n') if l.strip()]
        
        if len(lines) < 2:
            return [], "Texto insuficiente para extrair tabela de clientes"
        
        # Tentar detectar delimitadores (tab, | , ;)
        all_rows = []
        delimiter = None
        for delim in ['\t', '|', ';']:
        ...
        if delimiter:
            header = [p.strip() for p in lines[0].split(delimiter)]
            for line in lines[1:]:
            ...
            # Sem delimitador claro: tentar interpretar como linhas nome/valor
            # Ou retornar texto bruto para edição manual
            return [], (
                "Não foi possível identificar formato tabular na imagem. "
                "Texto extraído:\n\n" + text[:1000]
            )
        
        if not all_rows:
            return [], "Nenhum dado extraído da imagem"
        
        df = pd.DataFrame(all_rows)
        return _normalizar_dataframe_clientes(df), ""
    except ImportError:
        return [], "Bibliotecas pytesseract e/ou Pillow não instaladas"
    except Exception as e:
        return [], f"Erro ao processar imagem: {str(e)}"


# Mapeamento de nomes de colunas comuns para nomes padronizados
_COLUMN_MAP = {
    # NOME
    'NOME': 'NOME', 'NAME': 'NOME', 'NOME FANTASIA': 'NOME', 'NOME_FANTASIA': 'NOME',
    'CLIENTE': 'NOME', 'NOME CLIENTE': 'NOME', 'NOME DO CLIENTE': 'NOME',
    # RAZÃO SOCIAL
    'RAZAO SOCIAL': 'RAZAO_SOCIAL', 'RAZÃO SOCIAL': 'RAZAO_SOCIAL', 'RAZAO_SOCIAL': 'RAZAO_SOCIAL',
    'RAZÃO_SOCIAL': 'RAZAO_SOCIAL', 'RAZ_SOCIAL': 'RAZAO_SOCIAL',
    # EMAIL
    ...
    # CNPJ
    'CNPJ': 'CNPJ', 'CNPJ/CPF': 'CNPJ', 'CPF/CNPJ': 'CNPJ',
    # CPF
    'CPF': 'CPF',
    # CEP
    'CEP': 'CEP', 'CODIGO POSTAL': 'CEP', 'COD POSTAL': 'CEP', 'ZIP': 'CEP',
    # CIDADE
    'CIDADE': 'CIDADE', 'MUNICIPIO': 'CIDADE', 'MUNICÍPIO': 'CIDADE', 'CITY': 'CIDADE',
    'LOCALIDADE': 'CIDADE',
    # BAIRRO
    'BAIRRO': 'BAIRRO', 'BAIRRO/DISTRITO': 'BAIRRO', 'DISTRICT': 'BAIRRO',
    # NUMERO
    'NUMERO': 'NUMERO', 'NÚMERO': 'NUMERO', 'NUM': 'NUMERO', 'Nº': 'NUMERO', 'NO': 'NUMERO',
    # ESTADO
    'ESTADO': 'ESTADO', 'UF': 'ESTADO', 'STATE': 'ESTADO',
    # RUA
    'RUA': 'RUA', 'ENDERECO': 'RUA', 'ENDEREÇO': 'RUA', 'LOGRADOURO': 'RUA', 'ADDRESS': 'RUA',
    # COMPLEMENTO
    'COMPLEMENTO': 'COMPLEMENTO', 'COMPL': 'COMPLEMENTO',
}

# Todas as colunas esperadas no template
COLUNAS_TEMPLATE_CLIENTES = [
    'NOME', 'RAZAO_SOCIAL', 'EMAIL', 'SOURCE', 'TELEFONE',
    'CNPJ', 'CPF', 'CEP', 'CIDADE', 'BAIRRO',
    'NUMERO', 'ESTADO', 'RUA', 'COMPLEMENTO'
]


def _normalizar_dataframe_clientes(df) -> List[Dict[str, Any]]:
    """Normaliza um DataFrame para o formato padrão de clientes.
    
    Mapeia nomes de colunas variados para o padrão esperado.
    """
    import pandas as pd
    ...
    for col in df.columns:
        ...
        if col_upper in _COLUMN_MAP:
            rename_map[col] = _COLUMN_MAP[col_upper]
    ...
    df = df[[c for c in COLUNAS_TEMPLATE_CLIENTES if c in df.columns]]
